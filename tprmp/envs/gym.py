...
class Environment(gym.Env):
  ...
  def __init__(self,
    ...
    self.observation_space = gym.spaces.Dict({
        'task': gym.spaces.Box(low=np.array([0., 0., 0., 0., 0., 0., 0.], dtype=np.float32),
                               high=np.array([5., 5., 5., 1., 1., 1., 1.], dtype=np.float32),
                               shape=(7,),
                               dtype=np.float32),
        'configuration': gym.spaces.Box(low=np.array([0., 0., 0., 0., 0., 0., 0.], 0., dtype=np.float32),
                                        high=np.array([2*np.pi, 2*np.pi, 2*np.pi, 2*np.pi, 2*np.pi, 2*np.pi, 1.], dtype=np.float32),
                                        shape=(8,),
                                        dtype=np.float32),
        'ft_sensor': gym.spaces.Discrete(6)  # TODO: implement FT sensor later
    })
